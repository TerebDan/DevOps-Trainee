provider "aws" {
    access_key = AKIAS24HC3DBGSAWHEOK
    secret_key = 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
    region = eu-central-1
}
...
resource "aws_route53_record" "db1" {
  ...
  type    = "A"
  ttl     = 300
  records = [ aws_instance.db1.public_ip  ]
}
...
resource "aws_route53_record" "app_django_1" {
  ...
  type    = "A"
  ttl     = 300
  records = [ aws_instance.app_django_1.public_ip  ]
}
